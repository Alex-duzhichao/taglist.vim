...
function! s:Tlist_Window_Exit_Only_Window()
    ...
    " echo bufname(1). '  '. bufname(2). ' '. bufname(3). '  ' . nerdtree_winnum . ' ' . current_winnum
    " redir >> ~/vim_debug.output | echo nerdtree_winnum . ' ' . current_winnum | redir END
    if winbufnr(2) == -1 || (winbufnr(3) == -1 && winbufnr(2) == bufnr('NERD_tree_1'))
	" if winbufnr(2) == -1
	    if tabpagenr('$') == 1
